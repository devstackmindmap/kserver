...
{
    public class BattleRecorderRank : IBattleRecorder
    {
        ...
        private Dictionary<PlayerType, ProtoOnBattleResultRank> _protoBattleResults;
        private ProtoOnGetDeckWithDeckNum _protoDeckForBattle;

        public BattleRecorderRank()
        {
            _protoBattleResults = new Dictionary<PlayerType, ProtoOnBattleResultRank>(PlayerTypeComparer.Comparer);
        ...
        {
            var battleResult = AkaSerializer.AkaSerializer<ProtoOnBattleResultRank>.Deserialize(battleSendData.Data);
        ...
        private ProtoBattleRecordPlayer GetPlayerInfo(Player player, PlayerType enemyPlayer)
        {
            int teamRankPoint = 0;
            int userRankPoint = 0;
            int gettingRankPoint = 0;
            if ( true == _protoBattleResults.TryGetValue(player.PlayerIdentifier.Player, out var battleResult))
            {
                gettingRankPoint = battleResult.ChangedRankPoint;
                if (battleResult.UnitsRankData != null)
                    teamRankPoint = battleResult.UnitsRankData.Values.Select(rankData => rankData.CurrentSeasonRankPoint).Sum() - gettingRankPoint;
                if (battleResult.UserRankData != null)
                    userRankPoint = battleResult.UserRankData.CurrentSeasonRankPoint - gettingRankPoint;
            }
            ...
            var playerInfo = new ProtoBattleRecordPlayer
            {
                ...
                TeamRankPoint = teamRankPoint,
                UserRankPoint = userRankPoint,
                GettingRankPoint = gettingRankPoint,
