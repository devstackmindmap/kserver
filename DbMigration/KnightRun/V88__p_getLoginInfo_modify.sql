...
BEGIN
	...
	SELECT id, level, count, rankPoint, rankLevel, skinId FROM units WHERE userId = $userId;
	...
	SELECT id, performCount, receivedOrder, completedOrder FROM quests WHERE userId = $userId AND (receivedOrder =0 OR receivedOrder < completedOrder);
END|
DELIMITER ;