...
BEGIN
	SELECT gold, gem, gemPaid, level, exp, starCoin, soStartTicket FROM users WHERE userId = $userId;
	SELECT id, level, count, maxRankLevel, currentRankLevel, currentSeasonRankPoint, nextSeasonRankPoint, skinId ,
			 currentVirtualRankLevel, currentVirtualRankPoint
