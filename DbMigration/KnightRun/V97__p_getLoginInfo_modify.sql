...
BEGIN
	...
	SELECT objectExp, objectLevel, coreExp, coreLevel, agencyExp, agencyLevel FROM square_objects WHERE userId = $userId;
	SELECT userId, isActivated, activatedTime, nextInvasionTime, nextInvasionLevel, powerRefreshTime, objectPower, objectLevel, objectShield,
			 planetBoxExp, planetBoxLevel, invasionHistory FROM square_object_schedule WHERE userId = $userId;
END|
DELIMITER ;