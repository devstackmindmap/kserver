ALTER TABLE `asset_bundle`
	ADD COLUMN `fileExtensionType` CHAR(4) NOT NULL AFTER `Bytes`;
	
	
ALTER TABLE `game_table`
	ADD COLUMN `fileExtensionType` CHAR(5) NOT NULL DEFAULT '0' AFTER `Bytes`;
...
BEGIN
	...
	IF $fileType = 'Table' THEN
		SELECT originTable.version, originTable.name, originTable.url, originTable.runMode, originTable.mdChecksum, originTable.Bytes, originTable.fileExtensionType FROM (
