ALTER TABLE `asset_bundle`
	ADD COLUMN `mdChecksum` CHAR(50) NULL AFTER `orderNum`;
...
BEGIN
	...
	IF $fileType = 'Table' THEN
		SELECT originTable.version, originTable.name, originTable.url, originTable.runMode, originTable.mdChecksum  FROM (
