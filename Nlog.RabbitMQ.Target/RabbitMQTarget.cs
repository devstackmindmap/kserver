...
namespace Nlog.RabbitMQ.Target
{
	...
	public class RabbitMQTarget : TargetWithLayout
	{
        ...
        public ushort HeartBeatSecondsForDebugger { get; set; } = 300;
	...
	"Model not open to RabbitMQ instance", null);
		...
		private ConnectionFactory GetConnectionFac()
		{
			return new ConnectionFactory
			{
				...
				UserName = UserName,
				Password = Password,
				RequestedHeartbeat = System.Diagnostics.Debugger.IsAttached ? HeartBeatSecondsForDebugger : HeartBeatSeconds,
				Port = Port,
				Ssl = new SslOption()
				{
					Enabled = UseSsl,
		...
		{
			using (var gzipCompressedMemStream = new MemoryStream())
			{
