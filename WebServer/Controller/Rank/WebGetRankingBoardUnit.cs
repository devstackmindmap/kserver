...
{
    public class WebGetRankingBoardUnit : BaseController
    {
        public override async Task<BaseProtocol> DoPipeline(BaseProtocol requestInfo)
        {
            var req = requestInfo as ProtoRankingBoardUnit;
            ...
            using (var db = new DBContext("AccountDBSetting"))
            {
                ...
                var redisRankInfo 
                    = await AkaRedisLogic.GameBattleRankRedisJob.GetTopRanksKnightLeagueUnitAsync(redis, req.UnitId, serverSeasonInfo.CurrentSeason, req.CountryCode);
