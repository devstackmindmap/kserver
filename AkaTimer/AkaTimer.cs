...
namespace AkaTimer
{
    public class Timer : IDisposable
    {
        ...
        private void TimerIntervalChange(double forcingSetInverval)
        {
            ...
            Logger.Instance().Debug(Name + ":TimerIntervalChange:" + _timer.Interval);
        }
        ...
        private void AkaTimerStart()
        {
            if (_nowInterval > 0)
            {
                try
                {
                    if (_disposed == false)
                    {
                        _timer.Interval = _nowInterval; // Interval 값을 수정하면 내부에서 Start된다.
                        _timer.Start();                 // Interval 내부에서 Start되지만 명시적으로 Start를 호출해준다.
                    }
                    
                }
                catch(Exception ex)
                {
                    Log.Debug.Exception("AkaTimerStart", ex);
                    AkaLogger.Logger.Instance().Error($"[AkaTimerSTart] {ex.Message}\n{ex.StackTrace}");
                }
            }
        }
