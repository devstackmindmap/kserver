...
{
    public class SquareObjectCorePiece : Piece
    {
        public SquareObjectCorePiece(uint userId, DBContext db, int count = 0) : base(userId, count, "DontImplemnted", StoredProcedure.SQUAREOBJECT_CORE_LEVEL_UP, 0, db)
        {
        }
        ...
        public override async Task<DbDataReader> Select()
        {
            ...
            query.Append("SELECT isActivated, nextInvasionTime, coreLevel as `level` , coreExp as `count` FROM square_object_schedule  WHERE `userId`= ")
        ...
        {
            return AkaData.Data.GetSquareObjectPlanetCore(level).NeedExpForNextLevelUp;
        }

        public override int GetRequireGoldForLevelUp(uint pieceId, uint level)
        {
            return AkaData.Data.GetSquareObjectPlanetCore(level).NeedGoldForNextLevelUp;
        }
