...
{
    public class ItemExtractorCardPieceUnlockSelect : ItemExtractorPiece
    {
        public ItemExtractorCardPieceUnlockSelect(uint userId, PieceType pieceType, string tableName, DBContext db) : base(userId, pieceType, tableName, db)
        {
        }
        ...
        public override async Task Extract(DataItem dataItem, List<ProtoItemResult> alreadyAcheiveItems, uint itemValue)
        {
            ...
            using (var cursor = await piece.Select())
            {
                if (cursor.Read() == false)
                {
                    ...
                    await AddUserProductByUnlockCard(dataItem.ClassId);
                }
            }
        }
    }
