...
namespace KnightUWP
{
    ...
    sealed partial class App : Application
    {
        ...
        public App()
        {
            ...
            domain.UnhandledException += async delegate (object sender, System.UnhandledExceptionEventArgs e)
            {
                Exception ex = (Exception)e.ExceptionObject;
                StorageFolder storageFolder = ApplicationData.Current.LocalFolder;

                var fileName = $"exception_{DateTime.Now.ToString("MMdd_hhmmss")}";


                var file = await storageFolder.CreateFileAsync($"{fileName}_meta.csv", CreationCollisionOption.OpenIfExists);
                await FileIO.WriteTextAsync(file, e.ExceptionObject.ToString());
       ...
       //     rootFrame.Navigate(typeof(LoadingPage), null);

            rootFrame.Navigate(typeof(LoadingPage), null, new SuppressNavigationTransitionInfo());
        }
        ...
        protected override async void OnLaunched(LaunchActivatedEventArgs e)
        {
            ...
            if (rootFrame == null)
            {
                ...
                ApplicationView.GetForCurrentView().Title = "나이트런 클라테스터 - " + versionText;

                InitFrame();

                var info = await AutoUpdateManager.CheckForUpdatesAsync("http://172.30.1.224:8081/uwp/KnightUWP.appinstaller");

                if (info.ShouldUpdate)
                {
                    await new MessageDialog("업데이트가 있으니 종료합니다. 재시작하세요. ").ShowAsync();
            ...
            if (e.PrelaunchActivated == false)
            {
                if (rootFrame.Content == null)
                {
                    ...
                    rootFrame.Navigate(typeof(ProfileSelectPage), null);

                    //    rootFrame.Navigate(typeof(RootFrame), null);
        ...
        private void InitFrame()
        {
            ...
            if (ApiInformation.IsTypePresent("Windows.UI.ViewManagement.ApplicationView"))
            {
                ...
                if (titleBar != null)
                {
                    ...
                    titleBar.ButtonInactiveBackgroundColor = Colors.DarkBlue;
                    titleBar.ButtonHoverBackgroundColor = new Color() { A = 255, R = 19, G = 21, B = 40 };
               ...
               //     Windows.UI.ViewManagement.ApplicationView.PreferredLaunchViewSize = new Size(1200, 800);
